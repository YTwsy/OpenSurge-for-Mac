...
`omg connections` against the live external-controller API. It also restarts
mihomo in the same runtime directory to verify `profile.store-selected` restores
the selected policy. It proves the control-plane contract with mihomo, not
whole-LAN routing or transparent proxy capture.
