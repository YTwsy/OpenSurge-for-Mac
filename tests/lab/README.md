...
This lab keeps the gateway under test on macOS and uses two Lima Alpine VMs as
...
Optional proxy variables can be stored in `runtime/lab/proxy.env`. Both the
installer and all Lima lifecycle commands load that file automatically.
...
The lab sets `mihomo.redir_port` and `pf.redirect_tcp_to` to `0`. The current
Darwin mihomo build reports redir as unsupported, so transparent TCP capture is
kept out of the default passing loop until a supported TUN or engine-specific
