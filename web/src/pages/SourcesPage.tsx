  ...
  return <>
    ...
    <section className="section source-library">
      ...
      {sources.length ? <div className="source-grid">{sources.map(source => {
		...
		const state = t(sourceNeedsOverlay ? '附加配置待应用' : source.applied ? '运行版本' : source.desired ? running ? '待重载' : '下次启动版本' : previousApplied ? '新草稿' : source.valid && overlayCompatible ? '结构有效' : '存在冲突')
		const action = t(sourceNeedsOverlay ? running ? '应用附加配置并重载' : '保存附加配置到下次启动' : source.applied ? '已运行' : source.desired ? running ? '应用并重载网关' : '等待下次启动' : running ? '校验、应用并重载' : '设为下次启动版本')
        ...
        return <article className="source-card" key={source.id}>
		  ...
		  <div className={`source-validation ${overlayCompatible ? 'valid overlay' : 'invalid overlay'}`}><span aria-hidden="true">{overlayCompatible ? '✓' : '!'}</span><div><strong>{t(overlayCompatible ? '附加配置兼容' : '附加配置冲突')}</strong><small>{source.overlay_validation || t(overlayCompatible ? '当前来源可与附加配置组合' : '等待附加配置检查')}</small></div></div>
