...
export function DevicesPage({ overview }: { overview: Overview | null }) {
  ...
  const refresh = useCallback(async () => {
    try {
      const [devices, policies, policy] = await Promise.all([api.devices(), api.policies().catch(() => ({ groups: [] })), api.devicePolicy().catch(() => null)])
      setData(devices); setGroups(policies.groups); setPolicyDocument(policy); setError('')
  ...
  return <>
    ...
    {policyDocument ? <PolicyEditor document={policyDocument} onSaved={refresh} /> : <section className="section"><Empty text="当前 gateway config 尚未设置 device_policy.file；请先为该拓扑配置一个 OpenSurge 设备策略文件。" /></section>}
  </>
}

function PolicyEditor({ document, onSaved }: { document: DevicePolicyDocument; onSaved: () => Promise<void> }) {
  const [policy, setPolicy] = useState<PolicySet>(() => structuredClone(document.policy))
  ...
  useEffect(() => { setPolicy(structuredClone(document.policy)) }, [document])
  const list = (value: string) => value.split(',').map(item => item.trim()).filter(Boolean)
  const addProfile = () => {
    if (!profileID || policy.profiles.some(item => item.id === profileID)) return
    setPolicy({ ...policy, profiles: [...policy.profiles, { id: profileID, default_policies: list(profilePolicies), rules: [] }] })
    setProfileID('')
  }
  const addDevice = () => {
  ...
  return <section className="section policy-editor"><SectionTitle title="设备策略编辑器" subtitle="结构化编辑 desired policy；保存不会热改正在运行的 DHCP/mihomo bundle" />
    ...
    <div className="editor-grid"><div><h3>1. Profiles</h3><div className="form-row"><input aria-label="Profile ID" placeholder="profile id" value={profileID} onChange={event => setProfileID(event.target.value)} /><input aria-label="Profile 默认策略" placeholder="DIRECT, Proxy" value={profilePolicies} onChange={event => setProfilePolicies(event.target.value)} /><button onClick={addProfile}>添加</button></div>{policy.profiles.map(profile => <div className="editor-item" key={profile.id}><strong>{profile.id}</strong><span>{profile.default_policies.join(' / ')}</span><button onClick={() => setPolicy({ ...policy, profiles: policy.profiles.filter(item => item.id !== profile.id) })}>移除</button></div>)}</div>
      <DeviceRegistration policy={policy} device={device} setDevice={setDevice} addDevice={addDevice} setPolicy={setPolicy} />
      <RuleEditor policy={policy} rule={rule} setRule={setRule} addRule={addRule} />
    </div>
...
function RuleEditor({ policy, rule, setRule, addRule }: { policy: PolicySet; rule: RuleDraft; setRule: (value: RuleDraft) => void; addRule: () => void }) {
