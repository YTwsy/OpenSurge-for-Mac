...
export function NetworkPage({ overview, onChanged, onNavigate }: { overview: Overview | null; onChanged: () => Promise<void>; onNavigate: (page: 'devices') => void }) {
  ...
  const cleanupInterruptedRuntime = async () => {
    ...
    if (!window.confirm('确认安全清理上次运行留下的状态吗？OpenSurge 不会向旧 PID 发送信号，也不会更改本次开机的 PF 或 IPv4 forwarding。若系统代理仍指向 OpenSurge，会恢复启动前保存的设置。')) return
    setBusy(true); setError(''); setMessage('')
    try {
  ...
  return <>
    ...
    {config && gatewayInterrupted && <section className="section gateway-lifecycle-control interrupted-runtime-control">
      ...
      <div className="notice">若 macOS 系统代理仍指向 OpenSurge，本次清理会按已保存的所有权记录恢复启动前设置。</div>
    </section>}
