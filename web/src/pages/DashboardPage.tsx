...
export function DashboardPage({ overview, onOpenNetwork }: { overview: Overview | null; onOpenNetwork: () => void }) {
  ...
  return <>
    <PageHeader eyebrow="CONTROL CENTER" title="全屋网关，一眼可见" description="OpenSurge 负责网关生命周期；mihomo 是当前代理引擎。" action={<button className={running ? 'danger' : 'primary'} disabled={!overview} onClick={onOpenNetwork}>{running ? '停止网关' : '启动网关'}</button>} />
