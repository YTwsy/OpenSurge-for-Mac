...
function TailscaleDialog({ draft, setDraft, devices, authKeyPresent, identityPresent, busy, error, onCancel, onSave }: { draft: Draft; setDraft: (value: Draft | ((current: Draft) => Draft)) => void; devices: CompiledDevice[]; authKeyPresent: boolean; identityPresent: boolean; busy: boolean; error: string; onCancel: () => void; onSave: () => void }) {
  ...
  return <dialog className="tailscale-dialog" open aria-modal="true" aria-labelledby="tailscale-dialog-title">
	<div className="tailscale-dialog-head"><div><small>MANAGED TAILNET NODE</small><h2 id="tailscale-dialog-title">{t('配置 Tailscale 出站')}</h2><p>{t('OpenSurge 生成规则并交给 mihomo 内置 tsnet 数据面。')}</p></div><button type="button" aria-label={t('关闭 Tailscale 配置')} disabled={busy} onClick={onCancel}>×</button></div>
    <div className="tailscale-dialog-body">
	  <section className="tailscale-form-section identity"><div className="tailscale-section-copy"><span>01</span><div><h3>{t('节点身份')}</h3><p>{t('身份状态会持久化；普通重启和重载不会产生新设备。')}</p></div></div>
        <div className="tailscale-form-grid">
		  ...
		  <label className="wide"><span>Auth Key</span><input aria-label="Tailscale Auth Key" type="password" autoComplete="new-password" value={draft.auth_key ?? ''} onChange={event => update('auth_key', event.target.value)} placeholder={authKeyPresent ? t('已安全保存；留空保持不变') : identityPresent ? t('已有本地身份；可留空') : 'tskey-auth-…'} /><small>{t('仅写入 root 管理的受限文件，保存后不会回显；重新注册时单次密钥可能需要替换。')}</small></label>
        </div>
      </section>

	  <section className="tailscale-form-section"><div className="tailscale-section-copy"><span>02</span><div><h3>{t('Tailnet 目标')}</h3><p>{t('只有这些目标会进入 Tailscale；普通网页和订阅分流保持不变。')}</p></div></div>
	  ...
	  <section className="tailscale-form-section"><div className="tailscale-section-copy"><span>03</span><div><h3>{t('允许谁访问 Tailnet 目标')}</h3><p>{t('只控制上方域名、peer 和远端子网；Exit Node 的公网出口由“设备”页单独选择。')}</p></div></div>
	  ...
	  <section className="tailscale-form-section exit"><div className="tailscale-section-copy"><span>04</span><div><h3>{t('可选 Exit Node')}</h3><p>{t('填写后，这个托管出站才会成为可供设备选择的公网出口。')}</p></div></div>
		<label><span>{t('Exit Node 名称或 Tailscale IP')}</span><input aria-label="Tailscale Exit Node" value={draft.exit_node} onChange={event => update('exit_node', event.target.value)} placeholder={t('留空表示仅访问 Tailnet')} /></label>
		...
		<div className="tailscale-fail-closed"><span aria-hidden="true">◇</span><p><strong>Fail closed</strong> · {t('Exit Node 离线或目标不在 Tailnet 路由内时连接直接失败，不会从本地公网泄漏。')}</p></div>
      </section>
    </div>
	<div className="tailscale-dialog-footer"><label className="tailscale-enable"><input type="checkbox" checked={draft.enabled} onChange={event => update('enabled', event.target.checked)} /><span><strong>{t('保存后启用')}</strong><small>{t(draft.enabled ? '验证通过后立即重载运行中的网关' : '保存配置但不载入 Tailscale 节点')}</small></span></label><div><button type="button" disabled={busy} onClick={onCancel}>{t('取消')}</button><button className="primary" type="button" disabled={busy || !draft.display_name.trim() || !draft.hostname.trim() || !draft.control_url.trim()} onClick={onSave}>{t(busy ? '正在验证并应用…' : '保存 Tailscale 配置')}</button></div></div>
