...
export function PolicyGroupHealthCard({ group, search, healthByName, testing, onTest, onSelect, articleRef, navigationActive = false }: PolicyGroupHealthCardProps) {
  ...
  const options = useMemo(() => group.name.toLowerCase().includes(search.toLowerCase()) ? group.options : group.options.filter(option => option.toLowerCase().includes(search.toLowerCase()) || policyDisplayName(option, healthByName.get(option)).toLowerCase().includes(search.toLowerCase())), [group.name, group.options, healthByName, search])
  ...
  return <article className={`policy-health-group ${navigationActive ? 'is-nav-active' : ''}`} ref={articleRef}>
    <header className="policy-group-head"><div><span className="group-kicker"><span>{group.type}</span>{group.name.startsWith('device/') && <span>设备策略</span>}</span><h2>{group.name}</h2><p>当前出口 <strong>{group.selected ? policyDisplayName(group.selected, healthByName.get(group.selected)) : '未选择'}</strong>{!manual && ' · 自动策略组'}</p></div><div className="group-health-summary"><span><strong>{reachable}</strong> / {probeable.length} 可达</span><button type="button" disabled={!probeable.length || probeable.some(name => testing.has(name))} onClick={() => void onTest(probeable)}>检测本组</button></div></header>
    ...
    <div className="proxy-node-grid">{options.map(option => {
      ...
      return <button className={`proxy-node ${selected ? 'selected' : ''}`} type="button" key={option} aria-label={`${group.name} 选择 ${option}`} aria-pressed={selected} disabled={!manual || Boolean(switching)} onClick={() => void select(option)}>
        ...
        <span className="node-copy"><strong>{policyDisplayName(option, health)}</strong><small>{health?.selected && health.selected !== option ? `当前链路 → ${policyDisplayName(health.selected, healthByName.get(health.selected))}` : testedAgo(health?.tested_at)}</small></span>
