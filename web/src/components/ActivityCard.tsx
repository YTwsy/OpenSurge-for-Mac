...
export function ActivityCard({ traffic }: { traffic: DeviceTraffic | null }) {
  ...
  return <article className="activity-card">
    <header><div><small>ACTIVITY</small><h2>{t('当前活动')}</h2></div><span className="activity-pulse" aria-hidden="true" /></header>
    ...
    <span className="activity-total-label">{t('活跃连接')}</span>
    <div className="activity-breakdown">
      <span><strong>{localConnections}</strong><small>{t('本机连接')}</small></span>
      <span><strong>{attributedConnections}</strong><small>{t('已归属设备连接')}</small></span>
      <span><strong>{unidentifiedConnections}</strong><small>{t('待识别设备连接')}</small></span>
    </div>
  </article>
}
