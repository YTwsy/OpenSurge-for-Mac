...
func TestRecoveryTransitionsPersist(t *testing.T) {
	server := newTestServer(t)
	body, _ := json.Marshal(RecoveryUpdate{Stage: RecoveryPrepared, NetworkService: "Wi-Fi"})
	request := httptest.NewRequest(http.MethodPost, "http://127.0.0.1:61767/api/v1/recovery", bytes.NewReader(body))
	request.Host = "127.0.0.1:61767"
...
func TestRecoveryRejectsSkippedStage(t *testing.T) {
	server := newTestServer(t)
...
`), 0o600); err != nil {
	...
	server, err := New(Options{ConfigPath: configPath, Addr: "127.0.0.1:61767", StoreDir: filepath.Join(dir, "store"), Runner: fakeRunner{}, Static: http.NotFoundHandler()})
	if err != nil {
		t.Fatal(err)
	}
