...
OpenSurge for Mac 是一个正在持续开发的 Surge 风格 macOS 网关与控制面。它不只
代理 Mac 自己的流量，而是让 Mac 成为一个 IPv4 LAN：手机、平板、虚拟机、测试设备
和其他下游客户端可以从 Mac 获得 DHCP/DNS，并共享由策略控制的网络连接。

底层由 dnsmasq 提供 DHCP/DNS，mihomo 是当前代理引擎，macOS pf 与 IPv4
forwarding 提供原生网关路径。OpenSurge 是产品，mihomo 是它管理的引擎。项目正在
走向一个 Mac 原生、可审计的全屋代理网关，支持透明路由、可复现验证，以及更易用的
Web 与原生控制面。
...
当前网关核心仍是 CLI 驱动的 MVP，同时已经具备 loopback Web GUI 和原生菜单栏
launcher：
...
5. loopback Go Control API、内嵌 React Web GUI，以及只读 SwiftUI 菜单栏
   launcher。
...
- 通过 CLI 查看 imported mihomo proxy/rule provider 状态；
- 通过 CLI 查看当前 mihomo 连接状态；
- 在 Web GUI 中把当前活跃会话流量归属到 OpenSurge DHCP 设备，显示每设备连接数、
  实时上下行速率、活跃会话累计字节和占主要流量的 mihomo 出口链；
- 在 Web GUI 中集中查看和检测代理节点的可达性/延迟，并从同一健康视图切换已应用的
  Selector；
- 通过 applied mihomo mixed-port 路径检测固定真实服务目录，展示三轮中位延迟、
  命中规则和实际出口链；
...
`SRC-IP-CIDR` 规则。可选 JSON 策略文件支持设备默认出口、`REJECT` 这类设备专属
动作，以及按域名/IP/协议/端口/rule-provider 叠加的规则覆盖。
