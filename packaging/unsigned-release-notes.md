...
- 新增默认关闭的「Mac 本机系统代理协同」：TUN 模式下可同时启用 macOS HTTP/HTTPS 系统代理，兼容 SafeDNS、DNS Proxy 等 Network Extension 干扰 TUN-only 本机 DNS 的场景；启动冲突会 fail closed，停止、回滚与 mihomo 重启失败时恢复原状态；跨重启清理会恢复仍由 OpenSurge 持有的端点、保留用户已经修改的代理，并继续清掉旧 runtime；Desired 网络配置中的本机系统代理与每设备策略改用紧凑的状态开关。
...
- 修复 pkg 升级偶发需要安装两次：preinstall 先终止会自动唤醒 Control Service 的菜单栏 App，再循环卸载精确的用户级服务并处理迟到的替代进程；网关清理由新安装包自带的当前版本恢复 CLI 执行，不再依赖被替换版本的旧清理逻辑；仍保留 recovery 门禁和配置数据跨版本保留。
- Mac 睡眠唤醒或 Wi-Fi 重连后若本次开机的 Mihomo 已停止或本地 controller 拒绝连接，连通性页会只在该故障状态下显示「恢复 Mihomo」；健康运行和系统重启后的中断状态不会显示该入口。
...
- Added an off-by-default local system-proxy coordination option: in TUN mode OpenSurge can enable macOS HTTP and HTTPS proxies together for SafeDNS, DNS Proxy, and similar Network Extension conflicts; startup fails closed on existing proxy configuration, and stop, rollback, or failed mihomo restart restores the previous state. Cross-reboot cleanup restores endpoints still owned by OpenSurge, preserves proxy settings the user changed, and still clears the stale runtime; the local system-proxy and per-device-policy controls now use compact status switches in Desired Network Configuration.
...
- Fixed an intermittent pkg-upgrade failure that required a second install attempt: preinstall now stops the menu bar app that can wake the Control Service, repeatedly unloads the exact user service, and handles a late replacement process. Gateway cleanup now runs with the current-version recovery CLI embedded in the new package instead of depending on the version being replaced, while preserving recovery gates and existing configuration data.
- When Mihomo has stopped or its local controller refuses connections after a Mac wake or Wi-Fi reconnect while the current-boot gateway runtime is still active, Connectivity now shows a fault-only **Recover Mihomo** action. Healthy runtimes and runtimes interrupted by a full system reboot do not show it.
