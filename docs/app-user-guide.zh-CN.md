...
**选择或拖入 YAML**后选择，也可以直接拖到同一区域；这是页面中唯一的完整 YAML
配置来源入口。
...
需要在所有订阅之上长期保留自己的规则或节点时，可以展开来源列表之后默认折叠的
**高级：全局附加配置**。普通编辑只提供两项：固定放在订阅规则之前的高优先级自定义
规则，以及通过 `ss://`、`vmess://`、`vless://`、`trojan://`、`hysteria2://`、
HTTP 或 SOCKS5 分享链接添加的自定义节点；HTTP/SOCKS5 也保留简易手工表单。这里的
分享链接只会增加一个节点，不会新建订阅或完整 YAML 来源。

低优先级尾部规则、Provider、策略组和 DNS 操作仍受支持，但只在**专家 Overlay
YAML**中维护。保存只会生成附加配置草稿，并重新检查每个来源的名称冲突和高级引用。先通过
**最终配置预览**比较原始来源、附加变化、有效来源与最终 mihomo 配置，再对来源执行
应用。订阅刷新也只生成新草稿，不会自动改变正在运行的网关。
...
长驻进程继续使用已经失效的 fake-IP。需要为不同设备设置出口时，启用**每设备策略**。
完成后点击**保存网络配置**。
