...
IPv6 有两个独立设置：**IPv6 DNS 查询**控制是否回答 AAAA，**TUN IPv6**控制
这台 Mac 上的 mihomo VIF IPv6（关闭/自动/总是启用）。家庭使用优先选择**自动**；
只有明确知道宿主 IPv6 探测不适用时才选择**总是启用**。这两个设置不会向下游
局域网广播 RA，也不接管下游设备的 IPv6；客户端仍可能经原路由器 IPv6 绕过 Mac。
