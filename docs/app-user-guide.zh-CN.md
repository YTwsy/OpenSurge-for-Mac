...
需要在所有订阅之上长期保留自己的规则、节点或 Provider 时，可以在同一页面使用
**全局附加配置**。引导编辑支持前置规则、最终 `MATCH` 前的尾部规则、手工节点、
Provider、策略组扩展和常用 DNS 解析/过滤字段；复杂操作可以切换到高级 YAML。
保存只会生成附加配置草稿，并重新检查每个来源的名称冲突和引用。先通过
