...
把接口和 Mac 当前 IPv4 填入尚未保存的配置草稿；DHCP 接管还会在当前 `/24` 中生成一个
避开 Mac、路由器和受保护地址的建议地址池。自动填写不会保存配置，也不会修改 macOS
网络。**独立下游 LAN**保持手工配置，避免把上游地址误用到独立子网。

确认下游接口、上游接口、Mac 网关 IPv4、DHCP 地址池和上游 DNS。透明代理建议保持
**mihomo TUN**；需要为不同设备设置出口时，启用**每设备策略**。完成后点击**保存网络配置**。
