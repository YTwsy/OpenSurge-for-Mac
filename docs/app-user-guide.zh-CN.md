...
| 模式 | 适合场景 | 主要操作 |
| --- | --- | --- |
| 同一 LAN DHCP 接管 | 让同一家庭 LAN 的设备自动使用 OpenSurge | 需要按流程关闭和恢复路由器 DHCP |
| 同 LAN 手工网关 | 先让少量设备试用 | 路由器 DHCP 保持开启，设备手工把网关和 DNS 指向 Mac |
...
使用**同一 LAN DHCP 接管**时，启停操作位于网络恢复状态机中。请依次完成：
...
使用**同一 LAN DHCP 接管**时，停止网关也是恢复状态机的一部分：
