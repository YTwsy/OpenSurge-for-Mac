...
**直连主路由**，保存并重载网关。OpenSurge 仍为它保留并分配固定 IPv4，
但 DHCP 会向这个 MAC 单独下发主路由作为 Router 和 DNS。

应用后，请重新连接该设备的网络，使新的主路由网关和 DNS 生效。直连期间
该设备不再经过 OpenSurge，因此其代理、设备规则和流量统计暂停；Profile 和规则
不会被删除，切回**跟随网关规则**或**独立设备出口**并续租后恢复。
