...
- `make real-device-stop` 能停止 gateway，清理 runtime state，停止 DHCP 和
  mihomo，卸载 real-device PF anchor，并把 IPv4 forwarding 恢复为 disabled。
...
当前生成的 mihomo 配置仍是 `MATCH,DIRECT`。本轮真实设备 smoke 不证明订阅规则、
远端代理节点、策略组切换、出口 IP 变化或真实分流策略生效。后续如要验证这些
行为，应新增 proxy egress smoke，让 mihomo 使用可控代理节点或测试规则，并用
出口 IP、代理服务日志和 `mihomo.log` 交叉确认。
