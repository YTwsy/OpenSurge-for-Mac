...
这里的决策仍约束 IPv4 下游流量与 Mac 本机透明代理。独立下游 LAN 的实验性 IPv6
能力在物理 Ethernet ingress 使用 BPF → Unix sideband → Mihomo gVisor，以保留
source MAC；它不会重新启用 redir/PF redirect，也不改变 `transparent.mode: "tun"`
作为整体网关前置条件。详见[下游 IPv6 接管](downstream-ipv6-takeover.md)。
