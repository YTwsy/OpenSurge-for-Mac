...
和 `connections` 能通过 live external-controller API 工作，并会重启 mihomo 证明
`profile.store-selected` 可以恢复选中的策略。它适合策略组控制、机器可读 CLI、
mihomo API wrapper 和 `profile.store-selected` 相关改动；不要用它宣称 DHCP、DNS
下发、TUN 透明代理、same-LAN 或真实设备路径已验证。
